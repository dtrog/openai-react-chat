            ...
            <div ref={dialogRef}
              ...
              <div id='user-settings-content' className="flex flex-1">
                <div className="border-r border-gray-200 flex flex-col">
                  ...
                  <div
                      className={`cursor-pointer p-4 flex items-center ${activeTab === Tab.PROVIDERS_TAB ? 'bg-gray-200 dark:bg-gray-700' : ''}`}
                      onClick={() => setActiveTab(Tab.PROVIDERS_TAB)}>
                    <CpuChipIcon className="w-4 h-4 mr-3" aria-hidden="true"/>Providers
                  </div>
                  <div
                ...
                <div className="flex-1 p-4 flex flex-col">
                  ...
                  <div className={`${activeTab === Tab.PROVIDERS_TAB ? 'flex flex-col flex-1' : 'hidden'}`}>
                      ...
                      <div className="border-b border-token-border-light pb-3">
                        ...
                        <div className="mb-4">
                          ...
                          <select
                            ...
                            value={userSettings.activeProvider}
                            onChange={(e) => {
                              setUserSettings({
                                ...userSettings,
                          ...
                          >
                            ...
                            {userSettings.providerConfigs.map((config) => {
                              const registration = ProviderManager.getInstance().getProviderRegistration(config.name);
                              return (
                                <option key={config.name} value={config.name}>
                                  {registration?.displayName || config.name}
                                </option>
                              );
                            })}
                          </select>
                        ...
                        <div className="space-y-3">
                          <h4 className="text-md font-medium">Configured Providers</h4>
                          {userSettings.providerConfigs.length === 0 ? (
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                              No providers configured. Configure providers in your local.env.json file.
                            </p>
                          ) : (
                            <div className="space-y-2">
                              {userSettings.providerConfigs.map((config, index) => {
                                const registration = ProviderManager.getInstance().getProviderRegistration(config.name);
                                const isActive = userSettings.activeProvider === config.name;
                                return (
                                  <div 
                                    key={config.name} 
                                  ...
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="text-sm text-gray-500 dark:text-gray-400">
                                        {config.apiKey ? '••••' + config.apiKey.slice(-4) : 'No API Key'}
                                      </div>
                                    </div>
                                    {config.baseUrl && (
